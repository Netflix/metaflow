...
```{bash}
...
Workflow MovieStatsFlow triggered on AWS Step Functions (run-id sfn-b1bd0ec2-e29b-4818-90ad-507aa600dbf0).
...
```
Paste the run-id below (run_id = 'sfn-dolor-sit-amet') and run the following after the run finishes on Step Function.
```{r}
set_namespace(NULL)
run = flow_client$new('MovieStatsFlow')$run('sfn-b1bd0ec2-e29b-4818-90ad-507aa600dbf0')
print(run$steps)
```
