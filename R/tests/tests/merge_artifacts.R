...
test$merge_things <- decorated_function(
    function(self, inputs){
        ...
        assert_exception(expression(merge_artifacts(self, inputs)),
            "MergeArtifactsException")

        # Test to make sure nothing is set if failed merge_artifacts
        assert_exception(expression(print(self$non_modified_passdown)),
            "has no attribute")
        assert_exception(expression(print(self$manual_merge_required)),
            "has no attribute")
        ...
        assert_exception(expression(print(self$ignore_me)),
            "has no attribute")
    },
