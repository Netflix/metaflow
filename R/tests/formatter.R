...
choose_step <- function(test, name, node, graph_name){
    ...
    prio <- sapply(func_names, function(name){attr(test[[name]], "prio")})
...
flow_lines <- function(graphspec, test, context){
    ...
    for (name in names(graphspec$graph)){
        ...
        if ('foreach' %in% names(node)){
            lines <- c(lines, sprintf('self$%s <- %s', node$foreach_var, 
        ...
        if (is.null(step)){
            stop(paste("Test", test$name, "does not have a match for step", 
    ...
    func_names <- get_step_func_names(test)
    func_names <- Filter(function(name){attr(test[[name]], "required")}, func_names)
    for (func_name in func_names){
        step <- test[[func_name]]
        if (attr(step, "required") && !(func_name %in% used_test_func)){
            stop(sprintf("Test %s requires function %s but it was not matched for graph %s", 
    ...
    for (name in names(graphspec$graph)){
        ...
        if ('linear' %in% names(node)){
            ...
            branches <- paste(lapply(node$branch, 
                    function(name){sprintf('"%s"', name)}), collapse=",")
