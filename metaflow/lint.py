...
class FlowLinter(object):
    ...
    def run_checks(self, graph, **kwargs):
        for check in self._checks:
            if any(
                getattr(self, attr) or kwargs.get(attr) for attr in check.attrs
            ):
                check(graph)
...
def check_reserved_words(graph):
    ...
    msg = (
        "Step name *%s* is a reserved word. Choose another name for the "
        "step."
    )
    for node in graph:
...
def check_that_end_is_end(graph):
    msg0 = (
        "The *end* step should not have a step.next() transition. "
        "Just remove it."
    )
    msg1 = (
...
def check_unknown_transitions(graph):
    ...
    for node in graph:
        ...
        if unknown:
            raise LintWarn(
                msg.format(node, step=unknown[0]), node.tail_next_lineno
            )
