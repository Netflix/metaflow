...
class ArgoWorkflowsDeployedFlow(DeployedFlow):
    ...
    def resume(
        ...
        **kwargs,
        ...
        """
        Resume a failed run by re-using successful steps from the origin run.

        Parameters
        ----------
        origin_run_id : str
            Run ID of the original run to resume from.
        step_to_rerun : str, optional
            Step name from which to rerun (plus all downstream steps).
            If None, only previously failed steps are re-executed.
        **kwargs : Any
