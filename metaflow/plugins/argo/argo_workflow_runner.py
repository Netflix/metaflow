...
from metaflow.plugins.argo.argo_workflows_cli import remap_status
...
def get_lower_level_argo_group(api, top_level_kwargs: Dict, name: Optional[str]):
    if name is None:
        return getattr(api(**top_level_kwargs), "argo-workflows")()
    return getattr(api(**top_level_kwargs), "argo-workflows")(name=name)


class ArgoExecutingRun(object):
    def __init__(
        self,
        workflows_template_obj: "ArgoWorkflowsTemplate",
    ...
    @property
    def status(self):
        # TODO: (when status by Argo is Pending) OR (when status by Argo is Running but Run object is not available yet..) --> return Pending
        # TODO: underlying CLI should be consistent with what we return here..
        flow_name, run_id = self.pathspec.split("/")
        name = run_id[5:]
        status = ArgoWorkflows.get_workflow_status(flow_name, name)
        return remap_status(status)

    def terminate(self, **kwargs):
        _, run_id = self.pathspec.split("/")
        command = get_lower_level_argo_group(
...
class ArgoWorkflowsTemplate(object):
    def __init__(
        self,
    ...
    def from_deployment(name):
        # TODO: get the ArgoWorkflowsTemplate object somehow from already deployed argo-workflow, referenced by name
    ...
    def __get_executing_argo(self, tfp_runner_attribute, command_obj: CommandManager):
        try:
    ...
    def trigger(self, **kwargs):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            command_obj = self.runner.spm.get(pid)
            return self.__get_executing_argo(tfp_runner_attribute, command_obj)


class ArgoWorkflowsRunner(object):
    def __init__(
        self,
        flow_file: str,
        ...
        **kwargs
        ...
        # This is so that we can use ArgoWorkflowsTemplate.from_deployment(name=ArgoWorkflowsRunner("../try.py").name)
        self.name = name
    ...
    def __enter__(self) -> "ArgoWorkflowsRunner":
        return self
    ...
    def create(self, **kwargs):
        ...
        if command_obj.process.returncode == 0:
            return ArgoWorkflowsTemplate(runner=self)
        raise Exception("Error deploying %s to Argo Workflows" % self.flow_file)
