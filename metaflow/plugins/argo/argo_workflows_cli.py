...
@argo_workflows.command(
    help="Resume a failed run on Argo Workflows by re-using successful steps."
)
@click.option(
    "--origin-run-id",
    required=True,
    type=str,
    help="Run ID of the original run to resume from.",
)
@click.option(
    "--step-to-rerun",
    default=None,
    type=str,
    help="Step name to rerun (along with all downstream steps). "
    "If not specified, only failed steps are re-executed.",
)
@click.option(
    "--run-id-file",
    default=None,
...
def resume(
    ...
    **kwargs,
    ...
    # Compute which steps need to be re-executed
    steps_to_rerun = compute_steps_to_rerun(obj.graph, step_to_rerun)

    # Add resume parameters
    params["metaflow-resume-origin-run-id"] = origin_run_id
    params["metaflow-resume-steps-to-rerun"] = ",".join(sorted(steps_to_rerun))
    ...
    obj.echo(
        "Resuming workflow *{name}* on Argo Workflows "
        "(origin run *{origin}*, new run-id *{run_id}*).".format(
            name=workflow_name_to_deploy,
    ...
    if steps_to_rerun:
        obj.echo(
            "Steps to re-execute: %s" % ", ".join(sorted(steps_to_rerun)),
        )
    else:
        obj.echo("Only previously failed steps will be re-executed.")
