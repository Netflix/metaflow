...
def create(
    ...
    for node in obj.graph:
        ...
        if any([d.name == "slurm" for d in node.decorators]):
            raise MetaflowException(
                "Step *%s* is marked for execution on Slurm with Argo Workflows which isn't currently supported."
                % node.name
            )
        if any([d.name == "nvidia" for d in node.decorators]):
            raise MetaflowException(
                "Step *%s* is marked for execution on Nvidia with Argo Workflows which isn't currently supported."
                % node.name
            )
    validate_tags(tags)
