...
class TestUnboundedForeachInput(UnboundedForeachInput):
    """
    Test class that wraps around values (any iterator) and simulates an
    unbounded-foreach instead of a bounded foreach.
    """
    NAME = 'TestUnboundedForeachInput'

    def __init__(self, iterable):
        self.iterable = iterable
        super(TestUnboundedForeachInput, self).__init__()
...
class TestUnboundedForeachDecorator(StepDecorator):
    name = 'test_unbounded_foreach'
    results_dict = {}

    def __init__(self,
                 attributes=None,
                 statically_defined=False):
        super(TestUnboundedForeachDecorator, self).__init__(
    ...
    def control_task_step_func(self, flow, graph, retry_count):
        ...
        #foreach_var = flow._foreach_var
        #foreach_iter = getattr(flow, foreach_var)
        foreach_iter = flow.input
        if not isinstance(foreach_iter, TestUnboundedForeachInput):
            raise MetaflowException('Expected type to be '\
                                    'TestUnboundedForeachInput. Found %s'\
                                    % (type(foreach_iter)))
        ...
        for i in range(foreach_num_splits):
            # NOTE: This decorator doesn't work with `--metadata local` (and
            # hence disabled in *-all-local tests) since it requires
            # registration of string id(s) which the local metadata
            # service doesn't support (as of 09/16/2020).
