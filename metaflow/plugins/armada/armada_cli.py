...
@click.option(
    "--gpu-vendor", help="GPU vendor requirement for Kubernetes pod."
)
...
@click.option(
    "--use-tmpfs", is_flag=True, help="tmpfs requirement for Kubernetes pod."
)
@click.option(
    "--tmpfs-tempdir",
    is_flag=True,
    help="tmpfs requirement for Kubernetes pod.",
)
...
@click.option(
    "--shared-memory", default=None, help="Size of shared memory in MiB"
)
@click.option(
    "--port", default=None, help="Port number to expose from the container"
)
@click.pass_context
def step(
    ctx,
