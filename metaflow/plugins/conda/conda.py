...
class CondaStepException(CondaException):
    def __init__(self, exception, step):
        msg = "Step: {step}, Error: {error}".format(
            step=step, error=exception.message
        )
...
class Conda(object):
    ...
    def _remove(self, env_id):
        self._call_conda(
            ["env", "remove", "--name", env_id, "--yes", "--quiet"]
        )
    ...
    def _env_lock_file(self, env_id):
        return os.path.join(
            self._info()["envs_dirs"][0], "mf_env-creation.lock"
        )
        ...
        while True:
            try:
                ...
                if (time.time() - start) >= self.timeout:
                    raise CondaException(
                        "Timeout occurred while acquiring lock {}".format(
                            self.lock
                        )
                    )
