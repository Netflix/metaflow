...
def _get_external_card_packages():
    ...
    for m in get_modules("plugins.cards"):
        ...
        if getattr(m.module, "__file__", None):
            ...
            # This is to support current system where you have a namespace package and
            # then sub packages that have __init__.py
        ...
        try:
            ...
            for c in cards:
                ...
                if not getattr(c, "type", None):
                    # todo Warn user of non existent `type` in MetaflowCard
                    continue
