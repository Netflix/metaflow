...
class BatchClient(object):
    ...
    def describe_jobs(self, job_ids):
        for jobIds in [
            job_ids[i : i + 100] for i in range(0, len(job_ids), 100)
        ]:
            ...
            if swappiness is not None:
                if not (
                    ...
                    raise BatchJobException(
                        ...
                        "(should be 0 or greater and less than 100)".format(
                            swappiness
                        )
                    )
                else:
