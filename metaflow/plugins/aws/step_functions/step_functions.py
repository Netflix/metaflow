...
class StepFunctions(object):
    ...
    def list(cls, name, states):
        ...
        try:
            state_machine_arn = state_machine.get("stateMachineArn")
            return StepFunctionsClient().list_executions(
                state_machine_arn, states
            )
        except Exception as e:
            raise StepFunctionsException(repr(e))
        ...
        def _visit(node, workflow, exit_node=None):
            ...
            # Create a `Map` state and assign sub workflow if the node forks.
                ...
                cardinality_state = (
                    ...
                    .dynamo_db(
                        SFN_DYNAMO_DB_TABLE, "$.JobId", "for_each_cardinality"
                    )
                    .result_path("$.Result")
                )
                ...
                workflow.add_state(
                    ...
                    .iterator(
                        _visit(
                            ...
                            Workflow(node.out_funcs[0]).start_at(
                                node.out_funcs[0]
                            ),
                            node.matching_join,
                        )
    ...
    def _batch(self, node):
        ...
        if node.name == "start":
            ...
            if parameters:
                ...
                for parameter in parameters:
                    if parameter["value"] is not None:
                        default_parameters[parameter["name"]] = parameter[
                            "value"
                        ]
                # Dump the default values specified in the flow.
                env["METAFLOW_DEFAULT_PARAMETERS"] = json.dumps(
                    default_parameters
                )
            ...
            if node.is_inside_foreach:
                ...
                if any(
                    ...
                    and self.graph[self.graph[n].split_parents[-1]].type
                    == "foreach"
                    for n in node.out_funcs
                ):
                    env[
                        "METAFLOW_SPLIT_PARENT_TASK_ID_FOR_FOREACH_JOIN"
                    ] = attrs[
        ...
        batch_deco = [deco for deco in node.decorators if deco.name == "batch"][
            0
        ]
...
class State(object):
    ...
    def dynamo_db(self, table_name, primary_key, values):
        self.resource(
            "arn:%s:states:::dynamodb:getItem" % self._partition()
        ).parameter("TableName", table_name).parameter(
