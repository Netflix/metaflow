...
@parameters.add_custom_parameters(deploy_mode=False)
@step_functions.command(
    help="Resume a failed/stopped run on AWS Step Functions. "
    "Re-uses successful steps from the origin run."
)
@click.option(
    "--origin-run-id",
    required=True,
    type=str,
    help="Run ID of the run to resume (e.g., sfn-<execution-name>).",
)
@click.option(
    "--step-to-rerun",
    default=None,
    type=str,
    help="Name of a specific step from which to rerun. All downstream "
...
def resume(
    ...
    **kwargs,
    ...
    from metaflow.plugins.resume_util import compute_steps_to_rerun
    ...
    # Compute the set of steps to rerun.
    steps_to_rerun = compute_steps_to_rerun(obj.graph, step_to_rerun)
    steps_to_rerun_str = ",".join(sorted(steps_to_rerun))

    response = StepFunctions.trigger(
        obj.state_machine_name,
        params,
        resume_origin_run_id=origin_run_id,
        resume_steps_to_rerun=steps_to_rerun_str,
    )
    ...
    obj.echo(
        "Resuming origin run *{origin}* as *{run_id}* on AWS Step Functions...".format(
            origin=origin_run_id, run_id=run_id
        ),
        bold=True,
    )
    if steps_to_rerun:
        obj.echo(
            "Steps to rerun: %s" % ", ".join(sorted(steps_to_rerun)),
            indent=True,
        )
    else:
        obj.echo(
            "Only steps whose origin task was not successful will be rerun.",
            indent=True,
        )
