...
def create(
    ...
    obj.echo(
        "Deploying *%s* to AWS Step Functions..." % obj.state_machine_name,
        bold=True,
    )
    ...
    if project:
        if name:
            raise MetaflowException(
                "--name is not supported for @projects. "
                "Use --branch instead."
            )
...
def resolve_token(
    ...
    if prev_token is not None:
        ...
        if prev_user != get_username() and authorize != prev_token:
            ...
            obj.echo(
                "    step-functions create --authorize MY_TOKEN", fg="green"
            )
            obj.echo(
    ...
    obj.echo(
        "To analyze results of this production flow "
