    ...
    for node in obj.graph:
        if any([d.name == "snowpark" for d in node.decorators]):
            raise MetaflowException(
                "Step *%s* is marked for execution on Snowpark with AWS Step Functions which isn't currently supported."
                % node.name
            )
        ...
        if any([d.name == "nvidia" for d in node.decorators]):
            raise MetaflowException(
                "Step *%s* is marked for execution on Nvidia with AWS Step Functions which isn't currently supported."
                % node.name
            )
    validate_tags(tags)
