...
class LocalMetadataProvider(MetadataProvider):
    ...
    def default_info(cls):
        ...
        if v is None:
            return (
                "<No %s directory found in current working tree>"
                % DATASTORE_LOCAL_DIR
            )
    ...
    def register_data_artifacts(
        ...
        artdict = {
            "%d_artifact_%s" % (attempt_id, art["name"]): art for art in artlist
        }
    ...
    def register_metadata(self, run_id, step_name, task_id, metadata):
        ...
        metadict = {
            "sysmeta_%s_%d" % (meta["field_name"], ts): meta
    ...
    def _get_object_internal(
        ...
        if sub_type == "artifact":
            ...
            if meta_path is None:
                return result
            attempt_done_files = os.path.join(
                meta_path, "sysmeta_attempt-done_*"
            )
    ...
    def _make_path(
        ...
        if LocalStorage.datastore_root is None:
            ...
            LocalStorage.datastore_root = (
                LocalStorage.get_datastore_root_from_config(
