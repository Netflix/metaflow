...
class Channel(object):
    ...
    def send(self, obj):
        try:
            ...
            raise RuntimeError(
                "Cannot send object over streaming interface: %s" % e
            )
        except BaseException as e:
            raise ValueError(
                "Cannot serialize object: %s" % traceback.format_exc()
            )
    ...
    def recv(self, timeout=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot receive object over streaming interface: %s" % e
            )
        except BaseException as e:
            raise ValueError(
                "Cannot deserialize object: %s" % traceback.format_exc()
            )
