...
class S3(object):
        ...
        if run:
            ...
            if isinstance(run, FlowSpec):
                if current.is_running_flow:
                    prefix = os.path.join(
                        prefix, current.flow_name, current.run_id
                    )
                else:
    ...
    def info(self, key=None, return_missing=False):
        ...
        info_results = None
        try:
            _, info_results = self._one_boto_op(
                _info, url, create_tmp_file=False
            )
        ...
        for i in range(S3_RETRY_COUNT + 1):
            ...
            try:
                ...
                if error_code == 404:
                    ...
                    raise MetaflowS3URLException(
                        "Specified S3 bucket doesn't exist."
                    )
                error = str(err)
