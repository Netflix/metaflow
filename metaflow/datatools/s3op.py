                ...
                if mode == "info":
                    ...
                    try:
                        if url.range:
                            ...
                            resp = s3.get_object(
                                Bucket=url.bucket, Key=url.path
                            )
...
def process_urls(mode, urls, verbose, num_workers):
    ...
    if verbose:
        print(
            "%sing %d files.." % (mode.capitalize(), len(urls)), file=sys.stderr
        )
...
def verify_results(urls, verbose=False):
    for url, expected in urls:
        if verbose:
            print(
                "verifying %s, expected %s" % (url, expected), file=sys.stderr
            )
        try:
    ...
    if listing:
        for url, _ in urls:
            ...
            if idx_in_sz != len(to_load) and url.url == to_load[idx_in_sz].url:
                sz = (
                    sz_results[idx_in_sz]
                    if sz_results[idx_in_sz] >= 0
                    else None
                )
                idx_in_sz += 1
            if sz is None:
