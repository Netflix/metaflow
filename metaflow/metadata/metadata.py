...
class MetadataProvider(object):
    ...
    def _task_to_json(
        self, run_id, step_name, task_id=None, tags=[], sys_tags=[]
    ):
        d = {"run_number": run_id, "step_name": step_name}
    ...
    def _object_to_json(
        ...
        if obj_type == "task":
            return self._task_to_json(
                run_id, step_name, task_id, tags, sys_tags
            )
        if obj_type == "step":
    ...
    def _artifacts_to_json(
        ...
        for art in artifacts:
            ...
            d.update(
                self._all_obj_elements(self.sticky_tags, self.sticky_sys_tags)
            )
            result.append(d)
        return result
    ...
    def _register_code_package_metadata(
