...
def main(ctx):
    ...
    if ctx.invoked_subcommand is None:
        echo(
            "(%s): " % metaflow.__version__, fg="magenta", bold=False, nl=False
        )
    else:
...
def get_tutorial_metadata(tutorial_path):
    ...
    paragraphs = [
        paragraph.strip() for paragraph in content.split("#") if paragraph
    ]
...
@click.argument(
    "input_filename", type=click.Path(exists=True, resolve_path=True)
)
...
@configure.command(
    help="Configure metaflow to access self-managed AWS resources."
)
@click.option(
    "--profile",
    "-p",
