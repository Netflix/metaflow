    ...
    def __getattr__(self, name):
        if self._datastore and name in self._datastore:
            ...
            raise AttributeError(
                "Flow %s has no attribute '%s'" % (self.name, name)
            )
    ...
    def _validate_ubf_step(self, step_name):
        ...
        if join_type != "join":
            msg = (
                ...
                " The join type isn't valid.".format(
    ...
    def next(self, *dsts, **kwargs):
        ...
        if not (foreach is None or condition is None):
            msg = (
                ...
                "Specify either 'foreach' or 'condition', not both.".format(
                    step=step
                )
            )
            raise InvalidNextException(msg)
        ...
        if foreach:
            if not isinstance(foreach, basestring):
                msg = (
                    "Step *{step}* has an invalid self.next() transition. "
                    "The argument to 'foreach' must be a string.".format(
                        step=step
                    )
                )
                raise InvalidNextException(msg)
            ...
            if len(dsts) != 1:
                msg = (
                    "Step *{step}* has an invalid self.next() transition. "
                    "Specify exactly one target for 'foreach'.".format(
                        step=step
                    )
                )
                raise InvalidNextException(msg)
        ...
        if condition:
            if not isinstance(condition, basestring):
                msg = (
                    "Step *{step}* has an invalid self.next() transition. "
                    "The argument to 'condition' must be a string.".format(
                        step=step
                    )
                )
                raise InvalidNextException(msg)
