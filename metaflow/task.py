...
ForeachFrame = namedtuple(
    "ForeachFrame", ["step", "var", "num_splits", "index"]
)
...
class MetaflowTask(object):
    ...
    def _init_data(self, run_id, join_type, input_paths):
        ...
        if not ds_list:
            ...
            raise MetaflowDataMissing(
                "Input paths *%s* resolved to zero "
    ...
    def run_step(
        ...
        start = time.time()
        self.metadata.start_task_heartbeat(
            self.flow.name, run_id, step_name, task_id
        )
        try:
            # init side cars
            logger.start()
            ...
            if join_type:
                ...
                if join_type != "foreach" and len(inputs) != len(node.in_funcs):
                    raise MetaflowDataMissing(
                        ...
                        "were found"
                        % (step_name, len(node.in_funcs), len(inputs))
                    )
