...
class Parameter(object):
    def __init__(self, name, **kwargs):
        ...
        self.external_field = self.kwargs.pop('external_field', name)

        # external_artifact can be a function (returning a list), a list of
        # strings, or a string (which gets converted to a list)
        external_artifact = self.kwargs.pop('external_artifact', None)
        if callable(external_artifact) and not isinstance(external_artifact, DeployTimeField):
        ...
        if self.external_trigger and not self.external_artifact:
            raise MetaflowException(
                "Parameter *%s* has external_trigger=True "
                "but external_artifact is not specified. "
                "Specify the name of the external artifact." % name)
