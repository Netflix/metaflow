...
class Parameter(object):
    def __init__(self, name, **kwargs):
        ...
        if callable(default_field) and not isinstance(
            default_field, DeployTimeField
        ):
    ...
    def option_kwargs(self, deploy_mode):
        kwargs = self.kwargs
        if (
            isinstance(kwargs.get("default"), DeployTimeField)
            and not deploy_mode
        ):
            ret = dict(kwargs)
            ...
            ret["help"] = (
                help_msg + "[default: deploy-time value of '%s']" % self.name
            )
            ret["default"] = None
            ret["required"] = False
            return ret
