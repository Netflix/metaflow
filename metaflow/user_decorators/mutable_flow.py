...
from typing import Generator, Tuple, TYPE_CHECKING, Union
...
class MutableFlow:
    def __init__(
        self, flow_spec: "metaflow.flowspec.FlowSpec", pre_mutate: bool = False
    ):
        ...
        for var in dir(self._flow_cls):
            ...
            if callable(potential_step) and hasattr(potential_step, "is_step"):
                ...
                yield var, MutableStep(
                    self._flow_cls, potential_step, pre_mutate=self._pre_mutate
                )
    ...
    def add_parameter(
        ...
        if not self._pre_mutate:
            raise MetaflowException(
                "Mutating parameters is only allowed in the `pre_mutate` method and "
                "not in the `mutate` method"
            )
    ...
    def add_decorator(self, deco_type: Union[partial, str], **kwargs) -> None:
        ...
        if not self._pre_mutate:
            raise MetaflowException(
                "Mutating flow-level decorators is only allowed in the `pre_mutate` "
                "method and not in the `mutate` method"
            )
        ...
        if isinstance(deco_type, str):
            ...
            # Statically defined so we don't add it to the command line
            flow_deco.statically_defined = True
            # Check duplicates
            if (
    ...
    def __getattr__(self, name):
        ...
        if attr:
            ...
            if callable(attr) and hasattr(attr, "is_step"):
                return MutableStep(self._flow_cls, attr, pre_mutate=self._pre_mutate)
