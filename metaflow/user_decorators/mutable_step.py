...
class MutableStep:
    def __init__(
        ...
        self._mutable_container = MutableFlow(flow_spec, pre_mutate=pre_mutate)
    ...
    def add_decorator(
        ...
        if isinstance(deco_type, str):
            ...
            # We don't want to add it to the command line
            step_deco.statically_defined = True
            if isinstance(step_deco, StepDecorator):
                ...
                # User defined decorator. We only allow
                if not self._pre_mutate and isinstance(step_deco, StepMutator):
                    raise MetaflowException(
                        "Step mutators can only be added in the pre_mutate stage"
                    )
                debug.userconf_exec(
                    "Mutable step adding decorator %s to step %s (from str)"
                    % (step_deco.decorator_name, self._my_step.name)
                )
                # In __call__, the decorator will append itself to the right field to
                # self._my_step
                step_deco(self._my_step)
            return

        if issubclass(deco_type, UserStepDecoratorBase):
    ...
    def remove_decorator(self, deco_spec: str) -> bool:
        ...
        if not self._pre_mutate:
            raise MetaflowException(
                "Mutating step-level decorators is only allowed in the `pre_mutate` "
