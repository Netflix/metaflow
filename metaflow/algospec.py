...
needed in graph traversal, lint, runtime, or orchestrators.
...
class AlgoSpecMeta(FlowSpecMeta):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        # Class attributes with _is_algo_io=True (set by backend plugins)
        # are I/O schema descriptors, not artifacts. Exclude from
        # read-only protection and persistence.
        io_names = {
            n for n, v in attrs.items() if getattr(v, "_is_algo_io", False)
        }
        if io_names:
            cls._EPHEMERAL = cls._EPHEMERAL | io_names
            cls._NON_PARAMETERS = cls._NON_PARAMETERS | io_names
