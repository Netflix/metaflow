...
def write_latest_run_id(obj, run_id):
    ...
    if LocalStorage.datastore_root is None:
        LocalStorage.datastore_root = (
            LocalStorage.get_datastore_root_from_config(obj.echo)
        )
...
def compress_list(
    lst, separator=",", rangedelim=":", zlibmarker="!", zlibmin=500
):
...
def longest_common_prefix(lst):
    if lst:
        return "".join(
            a
            for a, _ in takewhile(
                lambda t: t[0] == t[1], zip(min(lst), max(lst))
            )
        )
    else:
        return ""
