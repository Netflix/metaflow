from metaflow import FlowSpec, Parameter, step, resources, argo, conda, conda_base
...
class MnistFlow(FlowSpec):
    ...
    @conda(libraries={'tensorflow': '2.3.0'})
    @step
    def start(self):
        """
        Load the dataset
    ...
    @argo(image='tensorflow/tensorflow:2.2.1-gpu-py3')
    @resources(gpu=1, cpu=2, memory=6000)
    @conda(disabled=True)
    @step
