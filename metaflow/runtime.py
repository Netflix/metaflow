...
class NativeRuntime(object):
    ...
    def _killall(self):
        ...
        while live_workers and int(time.time()) - now < 5:
            ...
            live_workers = [
                worker for worker in live_workers if not worker.clean()
            ]
        if live_workers:
            self._logger(
    ...
    def _queue_task_join(self, task, next_steps):
        ...
        if unbounded_foreach:
            ...
            matching_split = self._graph[
                self._graph[next_step].split_parents[-1]
            ]
            step_name, foreach_stack = task.finished_id
            ...
            if matching_split.type == "foreach":
                ...
                required_tasks = [
                    self._finished.get((task.step, s))
    ...
    def _queue_task_foreach(self, task, next_steps):
        ...
        if unbounded_foreach:
            ...
            self._queue_push(
                next_step,
                {"input_paths": [task.path], "ubf_context": UBF_CONTROL},
            )
        else:
...
class Task(object):
    ...
    def log(self, msg, system_msg=False, pid=None, timestamp=True):
        ...
        self._logger(
            msg, head=prefix, system_msg=system_msg, timestamp=timestamp
        )
        sys.stdout.flush()
    ...
    def _find_origin_task(self, clone_run_id, join_type):
        if self.step == "_parameters":
            ...
            origin_parent_pathspec = self.clone_pathspec_mapping[
                parent_pathspec
            ]
            parent = self.origin_ds_set.get_with_pathspec(
            ...
            if join_type == "foreach":
                ...
                index = ",".join(
                    [str(s.index) for s in foreach_stack]
...
class Worker(object):
    def __init__(self, task, max_logs_size):
        ...
        self.killed = (
            False  # Killed indicates that the task was forcibly killed
        )
        # with SIGKILL by the master process.
        # A killed task is always considered cleaned
        self.cleaned = (
            False  # A cleaned task is one that is shutting down and has been
        )
            ...
            if returncode:
                if not self.killed:
                    if returncode == -11:
                        ...
                        self.emit_log(
                            b"Task failed.", self._stderr, system_msg=True
                        )
            else:
