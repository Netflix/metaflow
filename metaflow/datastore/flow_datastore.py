...
class FlowDataStore(object):
    ...
    def __init__(
        ...
        """
        storage_impl = (
            storage_impl if storage_impl else self.default_storage_impl
        )
    ...
    def get_latest_task_datastores(
        ...
        if pathspecs:
            ...
            if steps:
                step_urls = [
                    self._storage_impl.path_join(run_prefix, step)
                    for step in steps
                ]
            else:
                step_urls = [
        ...
        with self._storage_impl.load_bytes(urls) as get_results:
            for key, path, meta in get_results:
                if path is not None:
                    _, run, step, task, fname = self._storage_impl.path_split(
                        key
                    )
                    ...
                    if fname == TaskDataStore.METADATA_DONE_SUFFIX:
                        ...
                        latest_started_attempts[(run, step, task)] = max(
                            latest_started_attempts.get((run, step, task), 0),
                            attempt,
                        )
