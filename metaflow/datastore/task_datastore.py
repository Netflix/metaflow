...
class TaskDataStore(object):
    ...
    def to_dict(self, show_private=False, max_value_size=None, include=None):
        ...
        for k, _ in self.items():
            ...
            if (
                ...
                and self._info[k]["size"] > max_value_size
            ):
    ...
    def _metadata_name_for_attempt(self, name, attempt_override=None):
        return self.metadata_name_for_attempt(
            name,
            self._attempt if attempt_override is None else attempt_override,
        )
        ...
        def blob_iter():
            for name, value in contents.items():
                if add_attempt:
                    ...
                    path = self._storage_impl.path_join(self._path, name)
                if (
                    isinstance(value, (RawIOBase, BufferedIOBase))
                    and value.readable()
                ):
                    yield path, value
                    ...
                    raise DataException(
                        "Metadata '%s' has an invalid type: %s"
                        % (name, type(value))
                    )
