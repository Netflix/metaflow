...
def _local_multinode_control_task_step_func(flow, env_to_use, step_func, retry_count):
    ...
    foreach_iter = flow._multinode_ubf_iter
    if foreach_iter.__class__.__name__ != "Multinode":
        raise MetaflowException(
            "Expected Multinode iterator object, got:" + foreach_iter.__class__.__name__
        )

    cluster_size = foreach_iter.cluster_size
    os.environ["MF_MULTINODE_NUM_NODES"] = str(cluster_size)
    os.environ["MF_MULTINODE_MAIN_IP"] = "127.0.0.1"
    ...
    # start workers
    subprocesses = []
    for node_index in range(1, cluster_size):
        task_id = "%s-node-%d" % (top_task_id, node_index)
        mapper_task_ids.append(task_id)
        os.environ["MF_MULTINODE_NODE_INDEX"] = str(node_index)
    ...
    os.environ["MF_MULTINODE_NODE_INDEX"] = "0"
    step_func()
