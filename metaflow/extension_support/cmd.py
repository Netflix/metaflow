...
def process_cmds_description(module_globals, is_extension=True):
    if is_extension:
        _process_cmds_description_ext(module_globals)
    else:
        _process_cmds_description_main(module_globals)
...
def _resolve_path(pkg_path, class_path):
...
def _process_cmds_description_ext(module_globals):
    # We want to modify all the relevans lists so that the relative paths
    # are made fully qualified paths for the modules
    _set_ext_cmds(
        module_globals,
        list(
            map(
                lambda p: (p[0], _resolve_path(module_globals["__package__"], p[1])),
                _get_ext_cmds(module_globals),
            )
        ),
    )


def _process_cmds_description_main(module_globals):
