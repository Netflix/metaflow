...
def process_aliases(module_globals):
    ...
    toggle_alias = []
    list_of_aliases = from_conf("ENABLED_ALIAS")
    ...
    try:
        ...
        for m in modules_to_import:
            for n, o in m.module.__dict__.items():
                ...
                elif n == "ENABLED_ALIAS":
                    list_of_aliases = o
            _resolve_relative_paths(m.module.__dict__)
            all_aliases.extend(_get_ext_aliases(m.module.__dict__))
    except Exception as e:
        _ext_debug("\tWARNING: ignoring all aliases due to error during import: %s" % e)
        print(
            "WARNING: Aliases did not load -- ignoring all of them which may not "
    ...
    for name, obj_path in all_aliases:
        _ext_debug("    Adding alias '%s' from '%s'" % (name, obj_path))
        all_aliases_dict[name] = obj_path

    # Resolve the ENABLED_ALIAS variable. The rules are the following:
    #  - if ENABLED_ALIAS is non None, it means it was either set directly by the user
    ...
    #  - if ENABLED_ALIAS is None, we populate it with everything included here and in
    ...
    _ext_debug("    Resolving metaflow aliases")
    ...
    for name in set_of_aliases:
        ...
        if obj_path is None:
            raise ValueError(
                "Configuration requested alias '%s' but no such alias is available"
                % name
            )
        ...
        try:
            ...
            raise ValueError("Cannot locate alias '%s' at '%s'" % (name, path))

        obj = getattr(alias_module, obj_name, None)
        if obj is None:
            raise ValueError(
                "Cannot locate '%s' object for alias at '%s'" % (obj_name, path)
            )
        _ext_debug("        Added alias '%s' from '%s'" % (name, obj_path))
