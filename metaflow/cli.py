...
@click.pass_obj
def dump(
    obj, input_path, private=None, max_value_size=None, include=None, file=None
):
...
@click.pass_obj
def logs(
    obj, input_path, stdout=None, stderr=None, both=None, timestamps=False
):
    types = set()
    if stdout:
        types.add("stdout")
                ...
                if any(data for _, data in logs):
                    ...
                    for line in mflog.merge_logs([blob for _, blob in logs]):
                        if timestamps:
                            ...
                            click.secho(
                                tstamp + " ", fg=LOGGER_TIMESTAMP, nl=False
                            )
...
def step(
    ...
    step_kwargs = dict(
        [
            (k[4:], v) if k.startswith("opt_") else (k, v)
    ...
    if pylint:
        ...
        if pylint.has_pylint():
            ...
            if pylint_is_happy:
                ...
                echo(
                    "Skipping Pylint checks.", fg="red", bold=True, indent=True
                )
        else:
            echo(
