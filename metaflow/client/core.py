...
def metadata(ms):
    ...
    if infos[0] in types:
        current_metadata = [
            m for m in METADATA_PROVIDERS if m.TYPE == infos[0]
        ][0]
        if len(infos) > 1:
...
class MetaflowObject(object):
    ...
    def __iter__(self):
        ...
        if children:
            return iter(
                sorted(children, reverse=True, key=lambda x: x.created_at)
            )
        else:
            return iter([])
...
class Task(MetaflowObject):
    ...
    def environment_info(self):
        ...
        env = [
            m
            for m in ENVIRONMENTS + [MetaflowEnvironment]
            if m.TYPE == env_type
        ][0]
