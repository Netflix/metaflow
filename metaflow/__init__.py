...
class _LazyLoader(object):
    ...
    def load_module(self, fullname):
        ...
        if isinstance(to_import, str):
        ...
        elif self._can_handle_orig_module() and isinstance(
            to_import, ModuleSpec
        ):
            ...
            raise ImportError(
                "Metaflow Extensions shadowed module '%s' does not exist"
                % fullname
            )
        else:
            raise ImportError
...
try:
    ...
    if addl_modules:
        ...
        lazy_load_custom_modules = {
            "metaflow.%s" % k: "metaflow_extensions.%s" % k
