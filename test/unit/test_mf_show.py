...
# Import mf_magics directly from its file so that the full metaflow package
# (and its platform-specific optional dependencies) is not triggered.
_MAGICS_PATH = (
...
_spec = importlib.util.spec_from_file_location("mf_magics", _MAGICS_PATH)
_mod = importlib.util.module_from_spec(_spec)
sys.modules.setdefault("mf_magics", _mod)
...
class TestMfShow(unittest.TestCase):
    def test_empty_line_prints_usage(self):
        ip = MagicMock()
        with patch.object(_mod, "IPython") as mock_ipy:
...
class TestLoadExtension(unittest.TestCase):
    def test_registers_magic_with_correct_name(self):
        ip = MagicMock()
        _mod.load_ipython_extension(ip)
