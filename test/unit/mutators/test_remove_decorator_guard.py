...
@pytest.fixture
def make_mutable_step():
    """Factory fixture to create MutableStep instances."""

    def _make(pre_mutate):
...
def test_remove_decorator_do_all_from_mutate_raises(make_mutable_step):
    ...
    ms = make_mutable_step(pre_mutate=False)
    with pytest.raises(MetaflowException, match="only allowed in the `pre_mutate`"):
        ms.remove_decorator("dummy_step_mutator")


def test_remove_decorator_specific_match_from_mutate_raises(make_mutable_step):
...
def test_remove_decorator_do_all_from_pre_mutate_succeeds(make_mutable_step):
    """From pre_mutate, removing the StepMutator via do_all should work."""
    ms = make_mutable_step(pre_mutate=True)
