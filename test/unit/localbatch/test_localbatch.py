...
Test layout mirrors the spin tests:
  - Batch REST API Tests  : validates every Batch REST endpoint using boto3.
                            No Docker required — jobs fail gracefully when unavailable.
  - Docker Execution Tests: validates actual container execution.
                            Skipped when Docker is not running.
  - Metaflow E2E Tests    : runs a real Metaflow flow via the @batch decorator
...
def _require_docker_if_marked(request):
    """Automatically skip any test marked with @pytest.mark.docker if daemon is unavailable."""
...
def test_health(localbatch_server):
    resp = requests.get(f"{localbatch_server.base_url}/health")
    assert resp.status_code == 200
    assert resp.json()["status"] == "ok"


def test_describe_queues_returns_default(batch_client):
...
def test_default_queue_is_healthy(batch_client):
...
def test_describe_compute_environments(batch_client):
...
def test_register_job_definition(batch_client):
    resp = batch_client.register_job_definition(
...
def test_revision_increments_on_re_register(batch_client):
...
def test_describe_job_definitions_by_name(batch_client):
...
def test_submit_job_returns_id_and_name(batch_client):
...
def test_job_reaches_terminal_state(batch_client):
...
def test_describe_jobs_returns_correct_shape(batch_client):
...
def test_terminate_job_transitions_to_failed(batch_client):
...
def test_list_jobs_returns_summary_list(batch_client):
...
def test_ecs_metadata_endpoint(localbatch_server):
...
@pytest.mark.docker
def test_successful_container(batch_client):
    arn = _register(
        batch_client,
        "docker-success-job",
...
@pytest.mark.docker
def test_failed_container_reports_exit_code(batch_client):
    arn = _register(batch_client, "docker-fail-job", command=["sh", "-c", "exit 42"])
    job_id = batch_client.submit_job(
...
@pytest.mark.docker
def test_inject_env_is_visible_inside_container():
...
# ---------------------------------------------------------------------------
# Metaflow End-to-End Tests (Mirrors Spin Test Style)
# ---------------------------------------------------------------------------
...
def test_batch_step_artifacts_are_persisted(simple_batch_run):
...
@pytest.mark.docker
@_NEEDS_CORE_BATCH_PARAMS
def test_run_succeeds(simple_batch_run):
    assert simple_batch_run.successful


@pytest.mark.docker
@_NEEDS_CORE_BATCH_PARAMS
def test_all_steps_have_tasks(simple_batch_run):
