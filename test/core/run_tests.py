...
def run_test(formatter, context, coverage_dir, debug, checks, env_base):
    ...
    try:
        ...
        shutil.copytree(
            os.path.join(cwd, "metaflow_test"),
        ...
        if "pre_command" in context:
            ...
            if pre_ret and not context["pre_command"].get(
                "ignore_errors", False
            ):
...
def produce_coverage_report(coverage_dir, coverage_output):
    ...
    try:
        ...
        if os.listdir("."):
            ...
            subprocess.check_call(
                COVERAGE + "html -d %s" % coverage_output, shell=True
            )
            log(
                "Coverage report written to %s" % coverage_output,
                real_good=True,
            )
        else:
    ...
    try:
        ...
        if coverage_output and not debug:
            produce_coverage_report(
                coverage_dir, os.path.abspath(coverage_output)
            )
