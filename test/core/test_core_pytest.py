...
def _run_flow(
    formatter, context, core_checks, env_base, executor, tmp_path, monkeypatch
):
    ...
    Process state (cwd + env) is managed by pytest fixtures:
      - tmp_path: per-test scratch dir, auto-cleaned (preserved on failure).
      - monkeypatch.chdir / monkeypatch.setenv: auto-unwound on test finalize,
        even if the test is interrupted (e.g. xdist worker SIGTERM).
    ...
    shutil.copytree(
        ...
        os.path.join(tmp_path, "metaflow_test"),
    )
    ...
    # Mirror env into the process via monkeypatch so anything that reads
    # os.environ later (Runner, dynamic flow import, checkers) sees it; pytest
    # unwinds these on test finalize, even on xdist worker SIGTERM.
    ...
    try:
        ...
        # We are already chdir'd to tmp_path so the path is reachable.
