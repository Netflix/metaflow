...
class PytorchLightningParallelTest(FlowSpec):
    ...
    num_nodes = Parameter(
        "num_nodes", help="Number of nodes in cluster", default=3,
    )
    num_local_processes = Parameter("num_local_processes", help='Number of local processes per nodde', default=2)
    ...
    def multinode_end(self, inputs):
        ...
        j = 0
        for input in inputs:
            assert input.reduced_tensor_value == sum(range(1, self.num_nodes * self.num_local_processes + 1))
            j += 1
