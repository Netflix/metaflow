...
class RandomFile(object):
    ...
    def checksum(self, start=None, length=None):
        if self.size is not None:
            ...
            length = (
                length if length and start + length < self.size else self.size
            )
...
def _format_test_cases(dataset, meta=None, ranges=None):
    ...
    for prefix, filespecs in dataset:
        objs = [
            RandomFile(prefix, fname, size) for fname, size in filespecs.items()
        ]
        ...
        if meta:
            ...
            for metaname, (content_type, usermeta) in meta.items():
                objs.update(
                    {
                        "%s_%s"
                        % (obj.url, metaname): (obj, content_type, usermeta)
                ...
                if size is not None:
                    ...
                    if meta is not None:
                        for metaname, metainfo in meta.items():
                            ...
                            if user_meta:
                                new_meta = {
                                    "metaflow-user-attributes": json.dumps(
                                        user_meta
                                    )
                                }
