...
def assert_results(
    s3objs, expected, info_should_be_empty=False, info_only=False
):
...
def test_list_paths(s3root, prefixes, expected):
    ...
    with S3(s3root=s3root) as s3:
        ...
        assert (
            frozenset(e.prefix.rstrip("/").split("/")[-1] for e in s3objs)
            == non_empty
        )

        for prefix, exp in matches.items():
            exists = frozenset(
                e.url for e in s3objs if e.prefix == prefix and e.exists
            )
...
def test_put_files(tempdir, s3root, blobs, expected):
    ...
    with S3(s3root=s3root) as s3:
        ...
        assert {s3obj.key for s3obj in s3objs} == {
            key for key, _ in shuffled_blobs
        }
